...
. Before continuing it is important to install the *systemd-resolved* package on your system using the appropriate *apt* command. This package will be used in a future step to get DNS working again on your system which will stop working temporarily after changing to the Networkd style configuration. The issue is that we need working DNS in order to download and install APT packages so we want to make sure we have installed this package before switching over our network configuration.
...
+
--
IMPORTANT: Once you stop `networking` and start `systemd-networkd` your system will let it's DHCP address lease expire. When that happens your system will loose it's DNS server address (which had come from DHCP) and you will no longer have the ability to look up domain names on the system. You will rectify this later in this section of the lab but make sure you do so as soon as possible!
--
...
[NOTE]
====
If you need to set the machine back to using DHCP after activating systemd-networkd you could do so with a networkd configuration file like:
[source,console]
----
[Match]
Name=ens18
...
====
--
. Although we set nameservers in the _/etc/systemd/network/10-primary.network_ file they are not actually in use yet. DNS servers have traditionally be stored in the _/etc/resolv.conf_ file which is edited manually when they need to be changed. However, this presents a problem for mobile devices which may need to frequently change DNS servers as they are connected to different networks. While there have been multiple schemes for doing this in the past including having the DHCP client software edit the _/etc/resolv.conf_ file or using an intermediate software package such as *openresolv* or *resolvconf* to manage this file with new-style networkd configuration there is also a new way to manage DNS settings using the *systemd-resolved* program.
+
--
NOTE: You should have previously installed the *systemd-resolved* package on your system using the appropriate *apt* command. If not you can try doing it now but if it has been too long since you disabled your DHCP address your system will have lost it's DNS server and will be unable to contact the Debian APT package servers to download and install software. If this happens you will need to temporarily set a manual DNS server in the `/etc/resolv.conf` file to `nameserver 172.17.50.1` in order to download and install `systemd-resolved`.
--

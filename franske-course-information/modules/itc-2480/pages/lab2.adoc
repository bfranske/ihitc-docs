...
. Check the contents of your _archive-experiments_ directory.
.. What happened? If you extracted a tar file made this way you could potentially end up with several more levels of directories than you really want. In this case we got an extra home/username directory inside of archive-experiments but if we were archiving something with an even deeper path we would have even more extra subdirectories. You can actually see this during the tar file creation if you have verbose output enabled. You saw that all the files being added to the tar had home/username/samplefiles/ in front of the filename. You will also see that tar printed a message "tar: Removing leading / from member names" before beginning. This all was once desired behavior when tar was primarily used for tape backups and you wanted the files restored to thier original locations. However, there are potential issues with this overwriting important system files when using tar files for exchanging with other people or between systems which is a much more common use today and why tar removes the leading / from the names (so they end up in a subdirectory from where you unpack the file rather than sprinkled around your system). However, we often do want to have unpacked files include creating the one single directory that they were packed from so they don't just all end up in the same directory as the tar file and remain grouped together. There are at least two ways to handle this which we will look at.
. Delete all files and subdirectories from inside the _archive-experiments_ directory.

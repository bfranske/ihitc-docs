...
. Unlike in our previous lab we want to carefully select from the many Docker Compose YAML files so we'll start `docker compose` with the `-f _<composeYAMLfile>_` option using `docker compose -f ./compose_pgsql.yaml up` as administrator.

...
Course Name: Network Security, Identity, and Authentication +
Course Number: ITC-2850-50 +
Instructor: Dr. Ben Franske +
Term: Fall 2026 (8/24/2026-12/17/2026) +
Meetings: T Th 12:00-1:50pm +
Location: B205
...
=== Prerequisite(s)
* ITC 2536 OR (ITC 2530 AND ITC 2800)
...
* Explain how network defense tools (firewalls, VPNs, secure overlay networks, IDS, etc.) are used to defend against attacks and mitigate vulnerabilities
* Implement firewall, routing, and network access translation technologies as a part of unified threat management
* Implement multiple virtual private network or secure overlay network technologies such as IPSEC, SSL/TLS VPNs, Wireguard, Tailscale, Nebula, Netbird, and Zerotier
...
* Explain and implement authentication methods such as username/password, passwordless/passkeys, single-sign-on, multi-factor authentication, bearer token authentication, JWT access tokens, certificate-based authentication, and basic HTTP authentication
* Explain the features of and implement identity and access management protocols such as LDAP, SAML, OpenID, OAuth, Kerberos, and RADIUS


== Course Design
This course will be taught in a lab seminar style where the class will be working collaboaratively with the instructor to explore different technologies through practical lab implementations. Portions of some units and activities may be completed online but others may require on campus lab work. Attendance at campus or online synchronous sessions is required unless it has been discussed with the instructor prior to the class session.

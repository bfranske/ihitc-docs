...
external_media_address=1.2.3.4 ; Replace with your public IP (check OpnSense)
external_signaling_address=1.2.3.4 ; Replace with your public IP (check OpnSense)
...
server_uri=sip:172.17.139.17 ; The IP address of the ITSP where we should register our IP address so they know where to send calls
; Update the username in the following line for your pod!
client_uri=sip:podX@172.17.139.17 ; The username we need to provide to register with the ITSP as well as the IP address of the ITSP registration server
endpoint=sipPSTN
line=yes
...
[sipPSTN]
type=aor
contact=sip:172.17.139.17:5060 ; All calls to the ITSP will go to this IP address and port

[sipPSTN]
type=identify
endpoint=sippstn
match=172.17.139.17 ; All calls from the ITSP will come from this IP address
----

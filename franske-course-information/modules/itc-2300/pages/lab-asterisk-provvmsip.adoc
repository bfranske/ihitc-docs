...
. https://sangomakb.atlassian.net/wiki/spaces/Phones/pages/21004585/Phones+-+Desk+Phone+Module+for+Asterisk+DPMA[The digium DPMA protocol] allows for automatic provisioning of specific Sangoma and Digium phones (S & D series phones) from Asterisk as well as access to many advaned application features on the phones.
. https://my.digium.com/en/login/register/[Sign up for an account at Sangoma/Digium] to use at the Digium store and after logging in https://store.digium.com/cart.php?action=add&product_code=804-00032&quantity=1[click here to "purchase" a free DPMA key].
. Install the *avahi-daemon* and *libavahi-client3* packages on your Debian server. These packages support Multicast DNS (aka mDNS or Bonjour) which is one way that the DPMA phones can find your Asterisk server and the method we will use. Because multicast traffic does not generally work outside of a single layer 2 subnet if you had your Asterisk server on a different subnet you would need to use an alternative method such as the https://sangomakb.atlassian.net/wiki/spaces/Phones/pages/23593101/Provisioning+D-Series#Configuration-when-Multicast-is-Not-Available[DHCP option 66 method].
. Get two Digium D60 phones. Do NOT connect them to the switch yet.
. Create a basic `res_digium_phone.conf` file for your phone with the correct MAC addresses and settingss for a xx04 and xx05 extension. See the xref:lab-asterisk-resdigiumconf.adoc[sample res_digium_phone.conf file] and the https://sangomakb.atlassian.net/wiki/spaces/Phones/pages/21397712/Phones+-+DPMA+Configuration[Sangoma official documentation] for some hints.
. Follow the https://sangomakb.atlassian.net/wiki/spaces/Phones/pages/21332083/Phones+-+DPMA+Installation[Asterisk DPMA installation instructions] to download the https://downloads.digium.com/pub/register/x86-64/register[registration program] (be sure to use the 64 bit one!) as well as to download the actual https://my.digium.com/en/docs/DPMA/dpma-download/?width=500&height=425&iframe=true[DPMA module] (again you want the 64 bit one for the Asterisk version you are running).
+
--
NOTE: Make sure to get the current DPMA module for the Asterisk version you are running. If you get one for a different version of Asterisk you will crash Asterisk when you try to load it.
--
. Follow the instructions to run the registration program and to get the DPMA module installed and activated.
. After copying the extracted modules you should restart the asterisk software to load the modules.
. Create the required global entries in your PJSIP file for DPMA configuration use. You may want to refer to https://sangomakb.atlassian.net/wiki/spaces/Phones/pages/20939676/Phones+-+Sangoma+Phones+when+used+with+DPMA#chan_pjsip[the Sangoma instructions for this]

...
ip dhcp pool internal-voip
   ...
   ! Change the IP on the next line to the IP address of your pods asterisk server (which is where the TFTP server is which this line points to)
   option 150 ip 172.17.55.XX
!
...
!
! Prevent any connections to the router's SIP port from the outside network EXCEPT from the Asterisk server
ip access-list extended block-sip-outside
 ! change the next line to have the correct IP for your pods asterisk server and outside IP address for your router
 permit udp host 172.17.55.XX host 172.17.144.XX eq 5060
 ...
 permit ip any any
!
! Prevent any connections to the router's SIP port from the inside
...
dial-peer voice 2 voip
 ...
 ! change the IP in the next line to be the IP of your Asterisk server
 session target ipv4:172.17.55.XX:5060

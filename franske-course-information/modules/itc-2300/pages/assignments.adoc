...
--
IMPORTANT: The Creating a network device list with a Python function lab lab has some errors and the get_device_interfaces.py script will not function correctly as of Fall 2024. Ben has submitted a patch to Cisco to fix this. If the fix has been made then the script will run, let Ben know so this notice can be removed. If it hasn't been made yet let Ben know and he can show you how to fix the issues.
--
IMPORTANT: The Cisco DNA Center Platform - Template Programmer lab has some errors and not all the scripts will run when you get to the point where you are encountering these errors go ahead and skip the rest of that activity.
--
...
+
--
NOTE: In the "Exploring IOS XE YANG Data Models with NETCONF" activity you run a command like `ssh -oHostKeyAlgorithms=+ssh-dss admin@sandbox-iosxe-recomm-1.cisco.com -p 830 -s netconf` which may present an error on some versions of SSH that no longer have the ssh-dss encryption algorithm compiled in. If this is the case for you continue reading but you will not be able to SSH into the IOS XE device yourself.
--
...
* https://learn.microsoft.com/en-us/training/modules/manage-virtual-machines-with-azure-cli/2-create-a-vm[Create a virtual machine with the CLI]
...
* The following two labs have technologies that have been replaced on Azure. You should follow the general outline of the scenario they give (creating the VM, etc.) but then switch to the new https://learn.microsoft.com/en-us/azure/update-manager/tutorial-assessment-deployment-using-policy[Tutorial: Enable periodic assessment and schedule updates on Azure VMs using policy] to actually monitor and schedule the updates you may also want to refer to the https://learn.microsoft.com/en-us/azure/update-manager/overview[About Azure Update Manager] page for additional information on this change.
...
+
--
NOTE: If you receive a message that an Azure zone like EastUS is out of capactity when trying to create the VM you will need to change your $LOCATION to be a different Azure zone (such as eastus2, centralus, westus2, or westus3) and then re-create the resource group, subnet, and attempt creating the VM in the new location.
--

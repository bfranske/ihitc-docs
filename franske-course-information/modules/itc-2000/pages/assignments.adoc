...
NOTE: Be sure to choose a new installation, not to repair an existing one. If prompted choose to install Windows "Education" or "Enterprise". Also, choose to do a "Custom" installation, not an upgrade, and delete any/all existing partitions on the drive (note that this will erase all data on the computer).
...
NOTE: You will need to download the Packet Tracer software from the https://archive.org/download/packet-tracer-822-64bit-setup-signed/Packet_Tracer822_64bit_setup_signed.exe[Internet Archive site] and copy it to your computer to install it.
...
+
--
NOTE: For Spring 2024 you can skip this lab activity. Microsoft has made some security changes to Windows which prevent this from working in many cases without changing many advanced options which are beyond the scope of the A+ certification.
...
+
--
NOTE: For Spring 2024 you can skip the second part of this lab activity. Microsoft has made some security changes to Windows which prevent this from working in many cases without changing many advanced options which are beyond the scope of the A+ certification.
--

=== Virtualization/Linux Labs (Module 9)
xref:lab-virtualization.adoc[Virtualization and Linux GUI exploration lab activity instructions]

=== Windows Installation Labs (Module 10)
xref:lab-wininstall.adoc[Windows Installation lab activity instructions]

=== Windows Configuration & Administration Labs (Module 11)
...
=== Mobile, Linux, and macOS Operating Systems Labs (Module 12)
* Complete pages 1-13 of the Linux Unhatched activity (accessed through the NetAcad site)
** To access the Linux Unhatched activity you need to enroll in the Linux UNhatched course using the link found on the front page for our class on D2L
** After enrolling and accessing the Linux Unhatched course on NetAcad you click on the "Course Content" link and then the "Enroll in NDG Online" link and complete the NDG registration including email verification
** Once you obtain access the required pages to complete are 1-13 and are found on the left side of the screen (pages 14-22.1 are not required for this class)

=== Security Labs (Module 13)
NOTE: All of these labs will be completed on one of the Netlab PCs you installed Windows 10 on in the previous lab.
...
=== Support and Scripting Labs (Module 14)
NOTE: The first part of this lab will be completed on one of the Netlab PCs you installed Windows 10 on in the previous lab. 

* 14.4.2.7 Lab - Write Basic Scripts in Windows and Linux
+
--
NOTE: The first part of this lab (Batch and PowerShell scripting) will be completed on one of the Netlab PCs you installed Windows 10 on in the previous lab. The second part of this lab (BASH scripting) will be completed in a Linux Virtual Machine. You will either need to install a Linux VM in your Netlab system or use the physical system with your Linux VM from a previous lab.
--
...
* Participate in an online forum discussion (typically 3 quality posts or more) of each module on the D2L site. See the xref:forum-posting:index.adoc[forum posting page] for details. These cannot be turned in late! (up to 10 points each module based on quality)
...
* Course Introduction Exam - Complete on the D2L Website (up to 20 points based on your score)
* Attend at least two Student Success Day presentations and bring proof of attendance signed by each presenter (10 points) -- Fall semester only
* Write a review of the PC History Video (10 points)
...
* Complete the A+ 220-1101 Certification Practice Exam (10 points for attempting)
* Complete the A+ 220-1102 Certification Practice Exam (10 points for attempting)
* Weekly Progress Self-Evaluation - Complete on the D2L Website, these cannot be turned in late (10 points possible each week)
...
TIP: Unit assessments will be completed on the Cisco Netacad website.

...
--
.. Use the web browser on your lab PC to navigate to http://172.17.139.70:8080/ which is where we'll download all files from in this lab activity. Click to download the latest Virtual Box setup program (ending with .exe) and Exptension Pack (ending with .vbox-extpack).
.. After the VirtualBox installation files are downloaded, run the installer (the file ending in .exe) and accept the default installation settings. You do *not* want to install the Python bindings as those will require additional software to be installed on your system and we don't need them for our usage of VirtualBox in this lab activity.
+
--
NOTE: If you are warned that you need the Visual C++ Redistributable files you can also download that from http://172.17.139.70:8080/ Because your host system is a 64-bit OS you'll want the x64 version. You could also download and install the file from Microsoft's web site if you prefer, an Internet search will assist you in finding it there.
--
...
.. Use the web browser on your lab PC to navigate to http://172.17.139.70:8080/ again if you closed the page. Click to download the latest Linux Mint CD image file. CD and DVD images typically end in .iso and are called "iso (ICE-O) files".

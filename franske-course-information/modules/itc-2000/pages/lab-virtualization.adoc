...
. The Lnux Mint OS is now installing in the virtual machine. This will take several minutes. When the Installation is complete message displays click "Restart Now" to restart the virtual machine. When prompted to remove the installation medium, press Enter to continue the booting process as the virtual CD will automatically be ejected.

...
== Part A — Prepare Devices for SSH Access
...
**Part A Complete!** Your devices are accessible via SSH and Netmiko is working. You're ready to start automating!

== Section 1 — Retrieving Show Command Output
...
**What happens in 2023 after IOS upgrade to 17.x:**
...
=== 1.4 Collecting from Multiple Devices
...
3. **Collect CDP neighbors information:**
   - Command: `show cdp neighbors`  
   - Create a topology map based on output
   - Document which devices are directly connected

4. **Interface error checking:**
...
== Section 2 — Sending Configuration Commands
...
=== 2.1 Single Configuration Command
...
=== 2.3 Bulk Configuration Across Multiple Devices
...
=== 2.4 Configuration from File (Templates)
...
[TIP]
====
**Configuration Templates:** In real environments, you'd use templates with variables (e.g., Jinja2 templates) to customize configurations per device. For example:
...
== Section 3 — Error Handling and Verification
...
=== 3.2 Configuration with Verification
...
== Section 4 — Practical Automation Scenarios

Real-world examples combining everything you've learned.

=== 4.1 Configuration Backup Script
...
=== 4.2 Deploy Standard Security Configuration
...
security_config = [
    ...
    # Enable secret (if not already set)
...
=== 4.3 Inventory Report Generation

[source, python]
----
# generate_inventory_report.py
# Create detailed inventory report from all devices
...
for device_name in all_devices:
    print(f"Collecting from {device_name}...", end=" ")
    ...
    try:
        ...
        version = connection.send_command("show version | include Software")
        hostname = connection.send_command("show running-config | include hostname")
        serial = connection.send_command("show version | include Processor board ID")
        uptime = connection.send_command("show version | include uptime")
        
        # Parse data (simplified parsing)
        device_info = {
            'Device Name': device_name,
            'IP Address': device['host'],
            'Hostname': hostname.split()[-1] if 'hostname' in hostname else device_name,
            'Software': version.strip(),
            'Serial Number': serial.split(':')[-1].strip() if 'Processor board ID' in serial else 'N/A',
            'Uptime': uptime.strip(),
...
# Save to CSV file
csv_filename = f"network_inventory_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"

with open(csv_filename, 'w', newline='') as csvfile:
    fieldnames = ['Device Name', 'IP Address', 'Hostname', 'Software', 'Serial Number', 'Uptime', 'Last Checked']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    
    writer.writeheader()
    for device_info in inventory_data:
        writer.writerow(device_info)

print("\n" + "=" * 80)
print(f"✓ Inventory report saved to: {csv_filename}")
print(f"  Devices processed: {len(inventory_data)}")

# Also print summary to screen
print("\nInventory Summary:")
for device_info in inventory_data:
    print(f"  {device_info.get('Device Name')}: {device_info.get('Hostname', 'ERROR')}")
----

=== 4.4 Practice Scenarios
...
1. **Interface Audit:**
   ...
   - Optionally: Auto-add descriptions to missing ones

2. **License Status Check:**
   - Command: `show license status`
   - Check all devices
   - Report which licenses are about to expire
   - Alert if any licenses are invalid
...
3. **Firmware Version Check:**
   ...
   - (Advanced: Download new IOS and upgrade)

4. **Configuration Compliance Check:**
   - Define required configuration elements (NTP, syslog, banner, etc.)
...
1. `test_netmiko_connection.py` — Test SSH connection to R1
2. `show_interface_status.py` — Retrieve interface status from R1
3. `collect_from_all_devices.py` — Collect data from all 4 devices
4. `configure_loopback.py` — Create loopback interface on R1
5. `configure_all_devices.py` — Apply common config to all devices
6. `backup_all_configs.py` — Backup configurations from all devices
7. `deploy_security_config.py` — Apply security best practices to all devices
8. `generate_inventory_report.py` — Create CSV inventory of network

=== Documentation Requirements

Create a document named `lab1_report.txt` or `lab1_report.pdf` containing:

1. **Device Configuration Evidence:**
   - Show running-config excerpts proving SSH is enabled on all devices
   - Usernames configured
   - Output from `show ip ssh` on each device

2. **Python Script Outputs:**
   - Screenshots or text output from running each required script
   - Show successful connection messages
   - Show configuration being applied
   - Show verification commands confirming changes

3. **Backup Files:**
   - Include the backup directory created by `backup_all_configs.py`
   - Show at least one full configuration file

4. **Inventory Report:**
   - Include the CSV file generated by `generate_inventory_report.py`

5. **Time Comparison:**
   - Estimate: How long would it take to manually configure all 4 devices with the security configuration?
   - Measure: How long did the Python script take?
   - Calculate time savings

6. **Answers to Reflection Questions** (see next section)

== Reflection Questions

Answer these questions in your `lab1_report` document:
...
=== Challenge 1: Parse Structured Data from Show Commands

Use the `textfsm` library to parse show command output into structured data:
...
# Use TextFSM parsing (built into Netmiko)
output = connection.send_command("show ip interface brief", use_textfsm=True)

# Now output is a list of dictionaries instead of raw text!
print(type(output))  # <class 'list'>
print(output[0])     # {'interface': 'GigabitEthernet0/0/0', 'ip_address': 'unassigned', ...}
...
Create a script that parses interface data and identifies:

- All interfaces with IP addresses
- All interfaces that are down
- All interfaces without descriptions

=== Challenge 2: Configuration Diff Tool

Before and after making changes, save configurations and use `difflib` to show what changed:
...
# Get config before changes
config_before = connection.send_command("show running-config").splitlines()

# Make changes
connection.send_config_set(['interface Loopback99', 'description CHANGED'])

# Get config after changes
config_after = connection.send_command("show running-config").splitlines()

# Generate diff
diff = difflib.unified_diff(config_before, config_after, lineterm='')
print('\n'.join(diff))

connection.disconnect()
----

=== Challenge 3: Interactive Device Selector

Create a script with a menu that lets you choose which device to configure:
...
for i, device_name in enumerate(all_devices, 1):
    print(f"  {i}. {device_name} ({devices[device_name]['host']})")
...
=== Challenge 4: Parallel Device Connections

Use Python's `concurrent.futures` to connect to multiple devices simultaneously:
...
def backup_device(device_name):
    ...
    # Save to file
    with open(f"{device_name}_backup.txt", 'w') as f:
        f.write(config)
    
    return f"{device_name} backed up"

# Execute backups in parallel (up to 4 simultaneous connections)
with ThreadPoolExecutor(max_workers=4) as executor:
    future_to_device = {executor.submit(backup_device, dev): dev for dev in all_devices}
...
This dramatically speeds up operations on many devices!

=== Challenge 5: Configuration Rollback
...
5. If connectivity fails, restores backup automatically

=== Challenge 6: Jinja2 Configuration Templates

Use Jinja2 templates to generate device-specific configurations:
...
# Send to device with Netmiko
# connection.send_config_set(config.splitlines())
----

=== Challenge 7: YAML-Based Device Inventory

Replace the Python dictionary with YAML file:

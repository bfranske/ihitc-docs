...
* https://crossconnect.com/posts/navigating-restconf-for-cisco-network-engineers/[Navigating RESTCONF for Cisco Network Engineers]
* https://networkop.co.uk/blog/2017/02/15/restconf-yang/[Introduction to YANG Programming and RESTCONF on Cisco IOS XE]
...
* **Cisco 4331 Routers** - R1 `10.10.20.10/24`, R2 `10.10.20.20/24` on GigabitEthernet0/0/0
...
! Configure the management interface (GigabitEthernet0/0/0)
...
! Verify interface is up: you should see "GigabitEthernet0/0/0 is up, line protocol is up"
Router(config)# do show ip interface brief
----

Repeat the same steps on R2 using IP `10.10.20.20/24` on GigabitEthernet0/0/0.
...
=== INTERFACE SUMMARY ===
GigabitEthernet0/0/0      enabled=true  ipv4=['10.10.20.10']
...
1. **Get a single interface:** Replace the URL with:
  `https://10.10.20.10/restconf/data/ietf-interfaces:interfaces/interface=GigabitEthernet0/0/0`
   
   Then verify with: `show running-config interface GigabitEthernet0/0/0`

2. **Get IPv4 addresses only on one interface:**
  `https://10.10.20.10/restconf/data/ietf-interfaces:interfaces/interface=Loopback0/ietf-ip:ipv4/address`
...
GigabitEthernet0/0/0   10.10.20.10     up        up
...
. The CLI `show vlan` command queries the same VLAN database
...
. Displays a menu: `[G]et, [C]reate, [M]odify, [D]elete, [Q]uit`
...
. **After each operation, remind the user to verify the change on the device CLI (display the appropriate `show` command to use)**
...
. **Verify via CLI using `show vlan brief` and `show running-config interface GigabitEthernet1/0/5`**
...
. **Both scripts should verify the interface creation using `show ip interface brief`**

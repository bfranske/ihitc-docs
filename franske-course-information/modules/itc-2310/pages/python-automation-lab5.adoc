...
. Advertise their networks (serial links, management network, and VLAN networks)
...
. **Verify on each switch:** Retrieve VLAN configuration from both S1 and S2 using your chosen method
. **Check consistency:** Do both switches have the exact same VLANs and names?
. **Generate a report:**
